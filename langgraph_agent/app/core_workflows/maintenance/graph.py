...
def ticket_creation_router(state: MaintenanceState):
    # A failed ticket (e.g. unresolved tenant -> Postgres FK error) has nothing left
    # for vendor_matching/human_approval/vendor_assignment to work with -- go straight
    # to response_generator, which already has an error branch (see ticket_creation_status
    # == "error" check there), instead of running three more nodes against no ticket.
...
def build_maintenance_graph(checkpointer=None):
    ...
    # Ticket Creation Node -> Vendor Matching (success) OR straight to Response Generator (failure).
    # This is the fix for the stuck-session bug: previously this was a single unconditional
    # edge to "vendor_matching" regardless of outcome, which meant a failed ticket still
    # walked through the whole vendor pipeline before response_generator ever saw the error.
    workflow.add_conditional_edges(
        "ticket_creation",
        ticket_creation_router,
        {
